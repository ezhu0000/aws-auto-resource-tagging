...
Resource Explorer 可能匹配到账号内任意可打标资源，但 **Lambda 仅对下表中的 ARN `service` 段** 调用 Resource Groups Tagging API，其余匹配结果会跳过并记入日志（`allowlist skipped`），避免对未授权或未列出的服务产生大量 403。**KMS（`kms`）** 故意不在下表中：CMK 打标依赖密钥策略，本方案不尝试给 KMS 打标。

**CloudFormation 堆栈**：通过统一 Tagging API 打标签时，AWS 侧会调用 `cloudformation:UpdateStack`（仅变更标签），故角色需同时具备 `DescribeStacks` 与 `UpdateStack`（模板中已授予）。
