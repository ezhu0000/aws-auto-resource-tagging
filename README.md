...
| EventBridge 托管规则 | 规则名含 `ConnectCampaignsRule` 或 `Managed.Connect` 时不允许打标（`ManagedRuleException`）。 |
| S3 Storage Lens | ARN 含 `storage-lens` 的资源需单独权限模型，此处跳过。 |

### 附录 A-2：KMS 与客户托管密钥（CMK）

若日志出现 **“no resource-based policy allows the kms:TagResource action”**：身份策略里已有 `kms:TagResource` 仍不够，**该密钥的密钥策略**必须允许本 Lambda 角色执行 `kms:TagResource`（或放宽给账号内角色）。AWS 托管密钥（aws/…）策略由 AWS 管理，部分场景仍可能拒绝；需在控制台或 CloudFormation 中调整对应 CMK 的密钥策略。
