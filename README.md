...
| EventBridge 托管规则 | ARN 形态为 `…:rule/规则名`（注意是 **`:`** 不是路径 `/rule/`）。规则名含 `ConnectCampaignsRule` 或 `Managed.Connect` 时不允许打标（`ManagedRuleException`）。 |
| S3 Storage Lens | ARN 含 `storage-lens` 的资源需单独权限模型，此处跳过。 |

**KMS（`kms`）**：本方案**不包含**在可打标范围内（CMK 依赖密钥策略，统一自动化成本高）；需要时请单独为密钥打标签或自行扩展模板。

### 附录 A-2：CloudFormation 堆栈打标

通过 Resource Groups Tagging API 更新堆栈标签时，除 `cloudformation:TagResource` 外，AWS 还可能校验 **`cloudformation:DescribeStacks`** 与 **`cloudformation:UpdateStack`**（模板已授予）。`UpdateStack` 能力范围较大，请确保仅通过本自动化角色使用；若有更严的合规要求，需自行收紧策略或从白名单/搜索范围中排除堆栈资源。
